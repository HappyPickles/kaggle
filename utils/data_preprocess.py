...
def poly_features(df: Union[pd.DataFrame, np.ndarray],
                  feature: List[str] = None,
                  degree: int = 2, norm: bool = True) -> pd.DataFrame:
    """
    ...
    assert isinstance(degree, (float, int)), TypeError('N must be a float or int.')
    assert isinstance(df, (pd.DataFrame, np.ndarray)), TypeError('df must be a Pandas DataFrame or NumPy array')
    if type(df) == np.ndarray:
        df = pd.DataFrame(df)
    if feature is None:
    ...
    polys = poly.fit_transform(df[feature])
    polys = pd.DataFrame(polys, columns=poly.get_feature_names_out())
    polys.drop(feature, axis=1, inplace=True)
    polys.drop(polys.columns[polys.apply(lambda x: all(x == 0))], axis=1, inplace=True)
    if norm:
        df = pd.concat([df, gaussian_norm(polys)], axis=1)
    else:
